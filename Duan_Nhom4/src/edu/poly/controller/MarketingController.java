...
public class MarketingController {
	...
	public String index(ModelMap model) {
		model.addAttribute("Marketing", new Sach());
		getMarketings(model);
		return "Marketing";
	}
	@ModelAttribute("marketings")
	public List<Sach> getMarketings(ModelMap model) {
		Session session = factory.getCurrentSession();
		String hql = "from Sach where madanhmuc = 'MDM003'";
		Query query = session.createQuery(hql);
		@SuppressWarnings("unchecked")
		List<Sach> list = query.list();
		model.addAttribute("marketings", phantrangcntt(vitrihientai,list)); //hiện danh sách đã phân trang 10 item 1 trang
	...
	// phân 10 item trên 1 trang. phải có vị trí để tính xuất 10 item thứ bao nhiêu
			...
			//-------------
				...
				public String danhmucpage(ModelMap model,@RequestParam("page")int page,@ModelAttribute("Marketing")Sach Marketing) {
					Session session = factory.getCurrentSession();
					String hql = "from Sach where madanhmuc = 'MDM003'";
					Query query = session.createQuery(hql);
					@SuppressWarnings("unchecked")
