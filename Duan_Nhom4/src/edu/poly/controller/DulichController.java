...
public class DulichController {
	...
	public String index(ModelMap model) {
		model.addAttribute("Dulich", new Sach());
		getDulichs(model);
		return "Dulich";
	}
	@ModelAttribute("dulichs")
	public List<Sach> getDulichs(ModelMap model) {
		Session session = factory.getCurrentSession();
		String hql = "from Sach where madanhmuc = 'MDM002'";
	...
	// phân 10 item trên 1 trang. phải có vị trí để tính xuất 10 item thứ bao nhiêu
			...
			//-------------
				...
				public String danhmucpage(ModelMap model,@RequestParam("page")int page,@ModelAttribute("Dulich")Sach Dulich) {
					Session session = factory.getCurrentSession();
					String hql = "from Sach where madanhmuc = 'MDM002'";
					Query query = session.createQuery(hql);
					@SuppressWarnings("unchecked")
