...
@Transactional
@RequestMapping("CNTT.poly")
@Controller
public class CNTTController {
	@Autowired
	SessionFactory factory;
	@RequestMapping()
	public String index(ModelMap model) {
		model.addAttribute("CNTT", new Sach());
		getCNTTs(model);
		return "CNTT";
	}
	@ModelAttribute("cntts")
	public List<Sach> getCNTTs(ModelMap model) {
		Session session = factory.getCurrentSession();
		String hql = "from Sach where madanhmuc = 'MDM001'";
		Query query = session.createQuery(hql);
		@SuppressWarnings("unchecked")
		List<Sach> list = query.list();
		model.addAttribute("cntts", phantrangcntt(vitrihientai,list)); //hiện danh sách đã phân trang 10 item 1 trang
	...
	// phân 10 item trên 1 trang. phải có vị trí để tính xuất 10 item thứ bao nhiêu
			...
			//-------------
				...
				public String danhmucpage(ModelMap model,@RequestParam("page")int page,@ModelAttribute("CNTT")Sach CNTT) {
					Session session = factory.getCurrentSession();
					String hql = "from Sach where madanhmuc = 'MDM001'";
					Query query = session.createQuery(hql);
					@SuppressWarnings("unchecked")
